...
import { X, Loader2, Utensils, Search, Coffee, Moon, Sun, Sunrise } from 'lucide-react';
...
function parseMenuMarkdown(markdown: string): Meal[] {
  ...
  for (const line of lines) {
    ...
    // Item (- **Name** (cal) _[Dietary]_)
       ...
       // Regex to extract parts: **Name** (cal) _[Dietary]_
       // Simple approach: Split by bold for name
...
export function MenuModal({ isOpen, onClose }: MenuModalProps) {
      ...
      fetch('/api/menu')
        ...
        .then(resData => {
            if(resData.content) {
                ...
                setData(parsed);
                // Set default tab to current meal (simple time logic or first available)
                // For now, default to the first meal found, e.g. Breakfast or current time logic
                // Let's rely on finding Lunch if it exists, otherwise first.
  ...
  const filteredStations = useMemo(() => {
    ...
    // Filter stations/items by search query
